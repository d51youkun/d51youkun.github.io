...
function userProfileSnapshot(user) {
...
async function fetchRemoteUserWithRetry(userId, attempts = 3) {
  let last = null;
  for (let i = 0; i < attempts; i++) {
    const remote = await cloudFetchUser(userId);
    if (remote && remote.id) {
      last = remote;
      if (!isPlaceholderUserName(remote.name)) return remote;
    }
    if (i < attempts - 1) await new Promise(r => setTimeout(r, 400 + i * 300));
  }
  return last;
}
...
async function cloudPushFriendship(id1, id2) {
  ...
  const users = {};
  const s1 = userProfileSnapshot(getUser(id1));
  const s2 = userProfileSnapshot(getUser(id2));
  if (s1) users[String(id1)] = s1;
  if (s2) users[String(id2)] = s2;
  for (let i = 0; i < 4; i++) {
...
async function syncFriendships() {
  ...
  for (const friendId of friendIds) {
    if (String(friendId) === String(user.id)) continue;
    const remoteUser = await fetchRemoteUserWithRetry(friendId);
  ...
  const batchProfilesUpdated = await syncFriendProfiles();
  if (added > 0 || profilesUpdated || batchProfilesUpdated) refreshMainUI();
  return added;
}
  ...
  for (const convId of convIds) {
    if (!data.conversations[convId]) {
      ...
      if (remoteConv && remoteConv.members && remoteConv.members.includes(user.id)) {
        ...
        if (!data.messages[convId]) data.messages[convId] = [];
        saveData(data);
        for (const memberId of remoteConv.members) {
          if (String(memberId) === String(user.id)) continue;
          fetchRemoteUserWithRetry(memberId, 2).then(remoteUser => {
            if (remoteUser && remoteUser.id) {
              ensureLocalUser(remoteUser);
              refreshMainUI();
            }
