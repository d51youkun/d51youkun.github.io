...
const SYNC_VERSION_KEY = 'bluechat_sync_version';
...
// ─── Force reload on server activity ───────────────────────
function startSyncVersionPolling() {
  if (syncVersionTimer) clearInterval(syncVersionTimer);
  if (!getSyncUrl()) return;
  const poll = async () => {
    const res = await cloudRequest('/api/sync-version');
    if (!res || res.version === undefined) return;
    const last = parseInt(localStorage.getItem(SYNC_VERSION_KEY) || '0', 10);
    if (last > 0 && res.version > last) {
      localStorage.setItem(SYNC_VERSION_KEY, String(res.version));
      location.reload();
      return;
    }
    localStorage.setItem(SYNC_VERSION_KEY, String(res.version));
  ...
  poll();
  syncVersionTimer = setInterval(poll, 3000);
}
