...
const SAFE_APP_ENHANCEMENTS = APP_ENHANCEMENTS.replace('const imageUrl=/^https://(?:[^/]+.)?googleusercontent.com//i.test(text)||/.(?:png|jpe?g|gif|webp|avif)(?:[?#].*)?$/i.test(text);', 'const rawText=String(m.content||"").trim();const leak=rawText.toLowerCase().includes("api.dicebear.com/7.x/thumbs/svg?seed=")||rawText.includes("alt=\"\">");if(leak)return "";const text=rawText;const cleanUrl=text.toLowerCase().split("?")[0].split("#")[0];const imageUrl=text.toLowerCase().startsWith("https://")&&(text.toLowerCase().includes("googleusercontent.com/")||[".png",".jpg",".jpeg",".gif",".webp",".avif"].some(ext=>cleanUrl.endsWith(ext)));');
