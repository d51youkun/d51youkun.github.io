...
const COMPOSER_UI = `<style>
#bt-composer-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;padding:10px 14px 8px;background:var(--bt-paper,#fff);border-top:1px solid #e2eaf3}
#bt-composer-actions button{min-height:44px;border:0;border-radius:13px;background:#edf4fd;color:#155ab2;font:700 13px system-ui,-apple-system,sans-serif;cursor:pointer}
#bt-composer-actions button:active{transform:scale(.98);background:#dbeafe}
.chat-input-bar #openStickerPickerBtn,.chat-input-bar [title="写真・動画・HTMLを送信"]{display:none!important}
html[data-bt-theme="dark"] #bt-composer-actions{background:var(--bt-paper,#172333);border-color:#2c415b}
html[data-bt-theme="dark"] #bt-composer-actions button{background:#223a58;color:#dcecff}
@media(max-width:480px){#bt-composer-actions{padding:8px 10px 6px}.chat-input-bar{padding-top:8px!important}}
</style><script>(function(){
function install(){var input=document.querySelector('#messageInput');if(!input)return;var bar=input.parentElement;if(!bar||document.querySelector('#bt-composer-actions'))return;var tray=document.createElement('div');tray.id='bt-composer-actions';tray.innerHTML='<button type="button" data-bt-composer="chat">💬 チャット</button><button type="button" data-bt-composer="sticker">😊 スタンプ</button><button type="button" data-bt-composer="photo">🖼️ 写真・ファイル</button>';bar.parentElement.insertBefore(tray,bar);tray.addEventListener('click',function(e){var action=e.target.closest('button')?.dataset.btComposer;if(action==='chat'){input.focus();return}if(action==='sticker'){document.querySelector('#openStickerPickerBtn')?.click();return}if(action==='photo'){var file=[...bar.querySelectorAll('input[type="file"]')].find(function(x){return x.accept&&x.accept.includes('image/')});file?.click()}})}
new MutationObserver(install).observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('DOMContentLoaded',install);install();
})();</script>`;
...
async function enhanceHtml(response) {
  ...
  return new Response(withManifest.replace('</body>', APP_ENHANCEMENTS + AVATAR_LEAK_GUARD + RECOVERY_SCRIPT + CALL_FETCH_GATE + RESPONSIVE_UI + COMPOSER_UI + '</body>'), { status: response.status, headers: { ...Object.fromEntries(response.headers), 'Cache-Control': 'no-store', 'X-BlueTalk-Source': 'genspark-ui-cloudflare-kv' } });
}
