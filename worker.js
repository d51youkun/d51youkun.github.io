...
const APP_ENHANCEMENTS = `<script>(function(){
  ...
  new MutationObserver(()=>{installDirectGateways();patchIceQueue();repairImages();addProfileTools();exactFriendSearch();addQrButton();bindAdminName();applyBadges();wireAdvancedChat()}).observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('keydown',adminTrigger);document.addEventListener('DOMContentLoaded',()=>{terms();installDirectGateways();patchIceQueue();repairImages();addProfileTools();exactFriendSearch();addQrButton();bindAdminName();wireAdvancedChat();if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{})});
})();</script>`;
