...
11. **メディアサイズ上限(MEDIA_SHIM v4)** — 画像・スタンプ・アバターのdataURLは150,000字超で `/bt-media` へ自動チャンク(再結合方式)。**動画・汎用ファイルは生ファイルを1.5MiBスライスで直接アップロード**(dataURL全体をメモリに持たない)+complete時に `sizeBytes/chunkBytes` を記録。GETは**ストリーミング応答**(チャンクを順次KV読み→ReadableStream、一括再結合しないため大容量でも503になりにくい)+**Range対応(206/Content-Range/シーク可能)**。上限: 動画・ファイル **200MB**、画像30MB(実測: 100MB往復合格・Range 206)。メッセージ取得はクライアントが開いている会話のみ `?bt_conv=` でサーバー側フィルタ(ポーリング負荷軽減=「サーバーエラー」の頻度低減)。未捕獲例外はCloudflareエラー画面ではなくJSON 500を返す。画面右下にビルドチップ(例: BT 0925-E)
