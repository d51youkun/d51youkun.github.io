...
11. **メディアサイズ上限(MEDIA_SHIM v3)** — 画像・動画・汎用ファイル・スタンプ画像・アバターのdataURLが150,000字超の場合、**/bt-media へ自動チャンク分割**で送るため実質サイズ上限は表記上限と一致。**実測で決めた上限は 12MB/ファイル**(20MBはGET再結合時にWorkerが503、12MBは3秒で合格)。画像はcanvas再圧縮(最大2560px・品質0.9、GIF/WebPは無変換)。ファイルフロー(media/generic/stickerのinput change)はdocument captureで横取りして上限チェック+チャンク送信を行い、app.jsの旧上限(動画6MB・スタンプ5MB)を無効化。画面右下に現在のビルドを示すチップ(例: BT 0925-D)を常時表示(キャッシュ切替の確認用)
