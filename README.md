...
4. **ダークモード補完(DARK_CSS + EARLY_THEME)** — トグル機構(属性 `data-bt-theme` + `bt_dark_mode`)は本体UIのものを使用し、配色ギャップのみ補完: ナビ・入力バー・通話オーバーレイ・Toast・ログインカード等。**早期適用スクリプト**が localStorage を初描画前に反映するため**ログインページでもダークモードが有効**(フラッシュなし)
